%! TEX program = xelatex
\documentclass[aspectratio=169,10pt,mathserif]{beamer}

\usepackage{graphicx,amsmath,amssymb,tikz}
\usepackage{pgfplots}   % Create plots from CSV files
\usepackage{csvsimple}	% reading CSV files in tables
\usepackage{booktabs}   % Nicer Tables
% \usepackage{enumitem}

% Use Helvetica light font
...
% \usecolortheme[rgb={0.13,0.28,0.59}]{structure}  % Blue titles
\usecolortheme[rgb={0,0,0}]{structure}  % Black titles
\setbeamertemplate{itemize subitem}{--}
\setbeamertemplate{frametitle} {
	\vspace{1em}
	\begin{flushleft}
	  {\huge\bf \insertframetitle}
	\end{flushleft}
}
...
\newcommand\footlineon{
  \setbeamertemplate{footline} {
    \begin{beamercolorbox}[ht=2.5ex,dp=1.125ex,leftskip=.8cm,rightskip=.6cm]{structure}
      \footnotesize % \insertsection  % Remove section name in slides
      \hfill
      {\insertframenumber}
    \end{beamercolorbox}
    \vskip 0.45cm
...
\title{\Huge \bfseries Slide Template}
...
\begin{frame}{Figure}
\begin{center}
Figure below shows an example of how to plot with PGFPlots
\begin{figure}
	\begin{tikzpicture}
		\begin{axis}
			[
